...
class FrameController {
  ...
  static async regeneratedFrames(req, res) {
    ...
    try {
      ...
      if (isFrameExist) {
        ...
        if (frameRegenerated.sequence === 0) {
          const sceneUpdateTime = await Scene.findByIdAndUpdate(
            frameRegenerated.scene,
            { updatedDate: new Date().toISOString(), thumbnail: responAws }
          );

          const scenesList = await Scene.find({
            project: sceneUpdateTime.project,
          });

          if (scenesList[0]._id.toString() === sceneUpdateTime._id.toString()) {
            await Project.findByIdAndUpdate(sceneUpdateTime.project, {
    ...
    try {
      ...
      if (isFrameExist) {
        await Frame.findByIdAndUpdate(request.frame_id, {
          activeUrl: request.active_id,
        });

        if (isFrameExist.sequence === 0) {
          const sceneUpdateTime = await Scene.findByIdAndUpdate(
            isFrameExist.scene,
            {
              updatedDate: new Date().toISOString(),
              thumbnail: isFrameExist.framesUrl[request.active_id],
            }
          );
      ...
      console.log(error, "update active url error");
  ...
  static async regenerateScene(req, res) {
    ...
    try {
      ...
      const data = await axios.post(
        "http://k8s-modelser-modelser-19b211bd7a-107069329.ap-south-1.elb.amazonaws.com/regenerate_scene",
        body,
        { timeout: 900000 }
      );
    } catch (error) {
